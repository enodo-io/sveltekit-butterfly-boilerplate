...
<svelte:head>
  ...
  {:else}
    ...
    <link
		rel="canonical"
		href={page.data.layer['content.type'] === 'post' && (page.data.scope as Butterfly.ApiResponse<Butterfly.Post>).data.attributes.canonical
...
<main id="page" class="mx-auto flex w-full max-w-screen-md flex-auto flex-col gap-7 p-2 md:py-7">
  {#if !page.error}
    <Breadcrumb />
  {/if}
